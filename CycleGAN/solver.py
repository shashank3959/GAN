...
class Solver(object):
    ...
    """ Builds the model (called during class initialization) """
    ...
    def train(self):
        ...
        # use this loss if use_labels = True
        ...
        for step in range(self.train_iters+1):
            ...
            if (step+1) % self.log_step == 0:
                ...
                # Print into Log file
                self.log_file.write('Step [%d/%d], d_real_loss: %.4f, d_mnist_loss: %.4f, d_svhn_loss: %.4f, '
                      'd_fake_loss: %.4f, g_loss: %.4f \n'
                      %(step+1, self.train_iters, d_real_loss.data[0], d_mnist_loss.data[0],
                        d_svhn_loss.data[0], d_fake_loss.data[0], g_loss.data[0]))
                self.log_file.flush()
